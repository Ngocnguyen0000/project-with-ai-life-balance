        ...
        LBCard(padding = LBSpacing.xl) {
            Column(verticalArrangement = Arrangement.spacedBy(LBSpacing.lg.dp)) {
                ...
                Column(verticalArrangement = Arrangement.spacedBy(LBSpacing.sm.dp)) {
                    Text("Create your account", style = LBTextStyle.heading1)
                    Text("Start tracking work and getting paid on time.", style = LBTextStyle.body)
                }

                LBTextField(label = "Full name", placeholder = "Ngoc Nguyen", value = fullName, onValueChange = { fullName = it }, icon = Icons.Filled.Person)
                ...
                LBPrimaryButton(title = "Sign up", onClick = { onSignUp(fullName, email, password) })

                Row(verticalAlignment = Alignment.CenterVertically) {
                    Text("Already have an account? ", style = LBTextStyle.body)
                    TextButton(onClick = onGoToLogin) {
                        Text("Log in", style = LBTextStyle.label.copy(color = LBColors.Accent))
                    }
                }
            }
        }
